...
async function checkUserAuthorization(user, pcSerial = null) {
    ...
    if (role === 'admin') {
        ...
        btnServerLua.style.display = ''; // VISIBLE SOLO PARA ADMIN
        configPanel.style.display = "none";
        ...
        btnConfig.style.display = 'none';
        btnServerLua.style.display = 'none'; // OCULTO PARA MODERATOR
        configPanel.style.display = "none";
        foldersContainer.style.display = "flex";
        if (statsRow) statsRow.style.display = 'flex';
        if (dashboardGrid) dashboardGrid.style.display = '';
...
window.showServerLua = () => {
    ...
    document.getElementById("luaCode").innerText = fullCode;
    document.getElementById("scrollArea").scrollTop = 0;
    updateLog("Server.lua completo cargado desde plantilla");
};
...
window.passServerLuaToUser = () => {
    ...
    if (!userOptionsHtml) {
        openPapeletaModal("ERROR", false, null, "", "No hay otros usuarios autorizados para enviar el código.");
        return;
    }
    ...
    document.getElementById("mTitle").innerText = "PASAR SERVER.LUA";
    msg.innerHTML = `Selecciona el usuario destino para enviarle el código completo de validación:`;
    
    input.style.display = "block";
    input.outerHTML = `<select id="mSelectUserLua" class="modal-input" style="display:block; text-align:left;">${userOptionsHtml}</select>`;
    
    modalAction = async (val) => {
        const select = document.getElementById("mSelectUserLua");
        const targetUser = select ? select.value : val;
        ...
        if (targetUser) {
            showServerLua(); // Muestra el código en el panel principal
            updateLog(`Codigo Server.lua listo para entregar a ${targetUser}`);
            openPapeletaModal("CODIGO LISTO", false, null, "", `El codigo completo ha sido cargado en el panel "Vista de Codigo".\n\nDestinatario: ${targetUser}\n\nPor favor, copia el cpdigo y enviaselo.`);
        }
    };
...
window.addLicense = async () => {
    if (!currentFolder) {
        return openPapeletaModal("ERROR", false, null, "", "️SELECCIONA UNA CARPETA PRIMERO");
    }
...
configLicense = {
    ...
    ["Key"] = "${keyValue}"
        ...
        card.innerHTML = `
            ...
            <!-- CAMPO IP:PUERTO CON BOTON EDITAR -->
