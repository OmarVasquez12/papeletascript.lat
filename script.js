...
// =============================================
//          PARTÍCULAS + DISCORD + FIREBASE + ETC.
// =============================================
...
function renderUsersList() {
    ...
    authorizedUsers.forEach(user => {
        ...
        if (user.id !== ADMIN_ID) {
            roleSelectorHtml = `
                <select class="role-selector" onchange="changeUserRole('${user.id}', this.value)" style="background: rgba(255,255,255,0.05); border: 1px solid var(--border-color); color: var(--text-main); padding: 0.4rem 0.6rem; border-radius: var(--radius-sm); font-size: 0.75rem; cursor: pointer; margin-right: 0.5rem;">
