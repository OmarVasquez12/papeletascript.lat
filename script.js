// =============================================
//          PROTECCIÓN ANTI-DEVTOOLS MEJORADA
// =============================================
...
(function() {
    ...
    const _config = {
        ...
        DISCORD_WEBHOOK: "https://discord.com/api/webhooks/1515757678935277818/NUG-PKCejzXNcKwIF0WKvugQ0H30Usyumki75uGr1cLD8fllMY-Rd5HYGm_db2ZbFCxb",
        DISCORD_WEBHOOK_LOGIN: "https://discord.com/api/webhooks/1515883928517611631/kLucfuf8QAAI6Q_r5OZ0zIvBQZ3GmRrzXM73SXgE_By7xnDaDlvU4RkXm57pqETmrY9n",
        DISCORD_CLIENT_ID: "1484013765878878378",
        ADMIN_ID: "890526767608127489"
    };

    // =============================================
    //          BLOQUEO TOTAL DE DEVTOOLS
    // =============================================
    
    // 1. Deshabilitar console completamente
    ...
    // 2. Detección por debugger + timing
    (function detectDevToolsByDebugger() {
        ...
        setInterval(function() {
            ...
            if ((end - start) > threshold) {
                document.body.innerHTML = '<div style="position:fixed;inset:0;background:#000;color:#f00;font-size:48px;text-align:center;padding-top:30vh;z-index:99999;">️ ACCESO DENEGADO ️<br><br>Herramientas de desarrollador detectadas</div>';
                setTimeout(function() { 
    ...
    // 3. Detección por dimensiones de ventana
    (function detectDevToolsBySize() {
        setInterval(function() {
            ...
            if (widthDiff > 150 || heightDiff > 150) {
                ...
                document.title = 'Acceso Bloqueado';
                window.location.href = 'about:blank';
            }
        }, 1000);
    })();
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // F5 y Ctrl+F5 (Recargar)
        if (e.key === 'F5' || (e.ctrlKey && e.key === 'F5') || (e.ctrlKey && e.key === 'r')) {
            e.preventDefault();
            e.stopPropagation();
            return false;
        }
    ...
    // 6. Detectar cuando se abre DevTools y recargar
    if (typeof DisableDevtool === 'function') {
        DisableDevtool({
            ondevtoolopen: function () {
                document.body.innerHTML = '<div style="position:fixed;inset:0;background:#000;color:#f00;display:flex;align-items:center;justify-content:center;font-size:36px;z-index:99999;"> ACCESO DENEGADO 🔒</div>';
                setTimeout(function() { 
    ...
    async function sendTelegramNotification(message) {
        if (_config.TELEGRAM_BOT_TOKEN === "TU_BOT_TOKEN_AQUI") return;
        
        try {
            const url = `https://api.telegram.org/bot${_config.TELEGRAM_BOT_TOKEN}/sendMessage`;
            await fetch(url, {
                method: 'POST',
    ...
    async function sendDiscordNotification(tipo, licencia) {
        try {
            ...
            await fetch(_config.DISCORD_WEBHOOK, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function sendDiscordLoginNotification(user, role, pcSerial) {
        try {
            ...
            await fetch(_config.DISCORD_WEBHOOK_LOGIN, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ embeds: [embed] })
            });
    ...
    async function checkUserAuthorization(user, pcSerial = null) {
        ...
        if (!userData && user.id !== _config.ADMIN_ID) {
    ...
    window.transferSelectedLicense = () => {
        ...
        document.getElementById("mMsg").innerHTML = `Selecciona el usuario destino para:<br><b>${license.resource}</b> (${license.ip})`;
    ...
    function renderLinks() {
        ...
        linksData.forEach(link => {
            ...
            const isAdmin = currentUser && currentUser.id === _config.ADMIN_ID;
